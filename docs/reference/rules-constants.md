# Глобальные правила: `rules.xml`

English: [rules-constants.en.md](rules-constants.en.md)

836 параметров в 61 разделах — числа, которые движок спрашивает по имени: экономика, бой, лечение, границы, победа, бонусы героев и фракций. У каждой записи проверено, встречается ли её имя среди строк исполняемого файла игры: имена параметров зашиты в код, и отсутствие имени означает, что запись не читает никто.
...
Тег раздела и тег подраздела вместе дают путь, который код спрашивает как
`economy/gather_and_commerce`; `name` — имя параметра внутри пути.

**`VALUE0`..`VALUE11` - не фиксированная ось, а массив до двенадцати слотов.**
Что означает слот, решает код и подсказывает `COMMENT`; осей встречается
минимум пять:

| Ось | Пример | Смысл слотов |
|---|---|---|
| ресурс | `timonium_buy` = `135 wealth`, `0`, `0`, `0 energy`, `0` | тимониум, богатство, очки исследований, энергия, пятый без подписи |
| уровень улучшения | `unit_heal_rate` = 1, 2, 4, 7, 10, 15, 20, 30 | восемь ступеней апгрейда |
| уровень героя | `giacomo_research_bonus` = 1..5 | пять уровней |
| размер города | `base_trade_value` = 5, 10, 15, 20, 25 | Site, Small City, City, Large City, Great City |
| счётчик | `diamond_los_bonus` = 0..6, дальше 6 | число подконтрольных бриллиантов |

Заполненность слотов:

| Заполнено слотов | Записей |
|---:|---:|
| 0 | 1 |
| 1 | 679 |
| 2 | 2 |
...
То есть 679 параметров из 836 — обычные скаляры, остальные массивы.

## Записи, которых код не спрашивает

У 11 записей имя не встречается в `legends.exe` ни разу. Значение в
файле есть, но получить его некому — правка такой строки не делает ничего.

| Раздел | `name` в файле | Что не так |
|---|---|---|
| `rules/territory` | `territory_push_factor` | имени нет в коде вообще |
| `rules/world` | `whatever` | имени нет в коде вообще |
| `tribes/vinci` | `vinci_politics` | имени нет в коде вообще |
| `tribes/cuotl` | `battery_sanctuary_extra_pushfactort` | опечатка; код спрашивает `battery_sanctuary_extra_push_factor` |
| `spells/interdict` | `interdict_attrition_multiplier` | имени нет в коде вообще |
| `spells/heartbeat_cooldown` | `hero_heartbeat_cooldown_magic ` | хвостовой пробел; код спрашивает `hero_heartbeat_cooldown_magic` |
| `spells/heartbeat_cooldown` | `hero_heartbeat_cooldown_tech ` | хвостовой пробел; код спрашивает `hero_heartbeat_cooldown_tech` |
| `spells/heartbeat_cooldown` | `unit_heartbeat_cooldown_magic ` | хвостовой пробел; код спрашивает `unit_heartbeat_cooldown_magic` |
| `spells/heartbeat_cooldown` | `unit_heartbeat_cooldown_tech ` | хвостовой пробел; код спрашивает `unit_heartbeat_cooldown_tech` |
| `spells/heartbeat_cooldown` | `dominance_heartbeat_cooldown ` | хвостовой пробел; код спрашивает `dominance_heartbeat_cooldown` |
| `music/music` | `music_ambient_sensitivity_duration` | код спрашивает только `music_peace_sensitivity_duration` |

Пять случаев с хвостовым пробелом — кандидаты, а не приговор: если разбор
обрезает пробелы у `name`, они работают. Проверяется разбором кода около
`stringlookup.cpp`; пока не проверено.
...
Код знает два раздела, которых в файле нет: `nations/korean` и
`nations/lakota` — следы Rise of Nations, предыдущей игры Big Huge Games.

## Все параметры

Пустые слоты опущены. Знак `−` в столбце «код» означает, что имени нет
в `legends.exe`.


### `economy/gather_and_commerce`

| код | `name` | Значения | Комментарий |
|:---:|---|---|---|
